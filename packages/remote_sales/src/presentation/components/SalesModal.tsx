...
import { SaleModalProps, StockProduct } from '../../domain/entities';
...
export default function SalesModal({ open, onClose, onSave, currentSale }: SaleModalProps) {
  ...
  const selectedProduct = useMemo(
    () => stockProducts.find((p: StockProduct) => p.id === productId),
  ...
  useEffect(() => {
    if (currentSale) {
      ...
      setDate(dayjs(currentSale.date));
    } else {
      setProductId('');
      setQuantity(1);
        ...
        <Grid container spacing={2}>
          ...
          <Grid size={12}>
            <DateInput
              label="Data da venda"
              value={date}
              onChange={(v: dayjs.Dayjs) => setDate(v)}
            />
          </Grid>
